...
#Dataset was taken from: https://docs.google.com/spreadsheets/d/19HV_7dWZzXpFsvVQXToTl2SrWT-uqio8oPxGTzameq8/edit?gid=0#gid=0
...
def genre(): #Code was written by partner.
    cate = input("What kind of category do you want to play? (adventure, fantasy, economic, or action): ") #Ask user for input on what game category they want to play
    if cate == "adventure": #Depending on what the user's category input, the computer recommends a random game from the category
...
def gametime(): #A board game is randomly generated based on the player's inputted game time. --- Code was written by me.
...
def menu(): #User will choose from the following to proceed in the process of being recommended a game. --- Written by collaboration between my code and my partner's code.
    while True: #Loops the menu until the user inputs break, to stop the loop.
        print ("---------------------------------------------------------------------------------------------------------")
